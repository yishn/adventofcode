// Part 1
w=~~Math.sqrt(I),w+=w%2;for(I=document.querySelector('.puzzle-input').innerText,x=y=i=0;i<(w+1)**2-I;i++){x+=i<w?1:i>=2*w&i<3*w?-1:0;y+=i>=3*w?-1:i>=w&i<2*w?1:0}Math.abs(w/2-x)+Math.abs(w/2-y)
